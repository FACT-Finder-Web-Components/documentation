...
| **restore-scroll-position** (Boolean) (default: "true") | When present, the record list will restore the scroll position, which user was on before being redirected to record page. **NOTE** To work correctly, attribute `[data-redirect]` has to be present inside `ff-record`. For more information, see the [Tracking](documentation/3.x/tracking-guide).   |
