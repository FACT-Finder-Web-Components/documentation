...
With the `ff-campaign-shopping-cart` you can retrieve shopping cart campaigns for a given set of record (product) ids. This element is meant to be used in conjunction with [`ff-campaign-pushed-products`](/api/3.x/ff-campaign-pushed-products) and [`ff-campaign-feedbacktext`](/api/3.x/ff-campaign-feedbacktext) and the `[is-shopping-cart-campaign]` attribute.
