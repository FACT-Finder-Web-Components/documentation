## Upgrade from version 4.x to 5.0.0

---

This guide documents the changes to the Web Components library between the `4.x` and `5.0.0` versions.
Changes to the `5.x` branch since `5.0.0` are not documented here.
See the changelog for details.

This article is not a step-by-step guide to walk you through the migration.
...
### Diagrams of request/response pipelines

There is now more focus on the request/response pipelines which shall provide more clarity of the data flow.
At the end of this migration guide you will find a visual representation of these pipelines.
These diagrams are a powerful aid whenever working with Web Components.
...
These methods take a required parameter which is equivalent to the FactFinder API specification for POST requests and an optional parameter to define various options to the request pipeline's behavior.
...
This is useful when setting up **page redirects** as the result of the FactFinder request from the redirecting page would be discarded anyway.
...
You have to create an API key first, then provide it to your Web Components application.
...
It will catch errors and skip to the next operation trying to keep the application running without becoming unusable.
...
If unset, the first FactFinder response that contains field roles will set it.
...
If the default formatters don't meet your requirements, you have the option to define your own.
...
Actions that Web Components will **not** take when `sandboxMode` is `true` include
...
document.addEventListener(`ffCoreReady`, ({ factfinder, init, initialSearch }) => {
    ...
    // This is also the place to invoke the search request replacing the `search-immediate` attribute.
...
Instead, you can toggle the "one-touch" behavior on by setting the `one-touch` attribute on `ff-slider`.
...
```html
<ff-recommendation product-number="123-456">
  <ff-record-list>
...
```html
<ff-similar-products id-type="productNumber" product-id="123-456">
  <ff-record-list>
    <template data-role="record">
      <ff-record>
        <!-- `Title` and `Price` are example fields that may exist in a data feed. -->
...
Depending on the `id-type` attribute, it either takes the _product number_ or the _master ID_.


#### Advisor campaign

`ff-campaign-advisor` property `campaignData` is now called `campaign` to mirror the name from the FactFinder API.
`campaignData` had been undocumented.

You can limit the campaigns to which the element shall react to with the attributes `name` and `not`.
Both take a comma-separated list of campaign names that you had defined beforehand in the FactFinder UI.
...
The `data-role="records-insertion-point"` template is no longer necessary and was removed.
...
You can specify them manually or use the provided filter builders.
...
> Note
>
> The _SearchParams_' `customParameters` field is neither parsed nor stringified.
...
See the [Request Pipelines](/api/5.x/request-pipelines) page for other request types.
