...
> Note
>
> When you manually load a page, e.g. from a URL with the 3rd page set in its search result, this will not scroll back to page 1 and 2.
> You have no option to load the first two pages anymore.
...
> Caution
>
> Do not use `infinite-scrolling` when the `ff-record-list` is inside a component like `ff-recommendation` or `ff-pushed-products`.
> The attribute is not designed to work in this context and using it may cause unwanted side effects.
...
> Note
>
> `infinite-scroll-container` searches for elements by using `document.querySelector()`.
> This means that when more than one container is found, only the first will be used.
> In case no scrollable container is found, `window` will be used.

### Placeholders
During the time `ff-record-list` is loading the next records, it can add placeholders to fill the empty space and increase the size of the scroll container.
This feature might increase application user experience as users see the blueprint of the next records batch almost immediately after they reach the `infinite-scroll-margin`.
As soon as the records are ready to be rendered, placeholders are removed and records are put in their place.
It is possible to customize the placeholder template by adding `data-container="infinite-scroll-placeholder"` to a specific container as shown in the example below.
...
> Note
>
> Please do not hide or change width and size of placeholders to `0px`.
> This could have negative side effects on the infinite scrolling feature.

### Restoring of scroll position
Infinite scrolling includes a built-in mechanism which allows to restore the user's last scroll position before moving to a record page. 
In order to make it work, the `restore-scroll-position` attribute needs to be added to the `ff-record-list` and the `data-redirect` attribute has to be present inside the `ff-record` element. For more information, see the [Tracking](/documentation/4.x/tracking-guide) section.

> Note
>
> At the moment, restoring the infinite scrolling position works only in single-page applications.
